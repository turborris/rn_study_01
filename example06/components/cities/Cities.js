...
import { Text, StyleSheet, View, Image, Platform, TouchableHighlight, TouchableWithoutFeedback, ScrollView } from 'react-native'
        ...
        return (
            <ScrollView contentContainerStyle={[!cities.length && { flex: 1 }]}>
                <View style={[!cities.length && { justifyContent: 'center', flex: 1 }]}>
                    ...
                    {
                        cities.map((item, idx) => (
                            <TouchableWithoutFeedback onPress={() => this.navigate(item)} key={idx}>
